...
- `StageDefinitionService` — 16 stages, in order. What a stage *requires* is data (`stage_document_requirement` + `document_type_fields`), never code: adding a field or document to a stage is a data change. **Document uploads are optional on every stage** (changeset `048`) — they still feed OCR when present, but they do not block Complete. The stage panel shows fillable catalogue fields immediately (including OCR-sourced ones such as Stage 3 Number of Bidders); repeating rows stay on their own tables. An APP workbook import still files the workbook as the APP Document.
- `StageEngine` — the **only** writer of `package_stage.status`. Completion is computed (mandatory scalar fields confirmed, validations pass; documents do not block), never simply set; a not-ready completion returns the list of reasons. Repeating catalogue fields (BER bidder rows, deliveries, invoices, payments, inspections) are gated on their own tables, not `extracted_field`.
